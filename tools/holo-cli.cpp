...
//   holo run  <ref> [--prompt T] [-n N] ensure local, verified-load, generate
...
static void ensure_dirs() {
    ...
    _mkdir(r.c_str());
    for (const char * d : { "/blobs", "/manifests", "/tensors", "/refs" }) _mkdir((r + d).c_str());
}
...
static int cmd_run(const resolved & r, const std::string & prompt, int n_gen) {
    ...
    printf("\n");
    fprintf(stderr, "[%8.1fms] done — %d tokens  ⟡ ran holo:b3:%.16s…\n", ms(), n_gen, r.b3.c_str());
